...
- What is the advantage of a jump box?_
    - Advantage of a jump box is to enable installation of DVWA and ELK containers so an user can visualize the logs that are created on the server.
...
- What does Metricbeat record?
    - Metricbeat is a lightweight agent that can be installed on target servers to periodically collect metric data from your target servers, this could be operating
      ...
      system. 
    - Yadav, V. (2021, July 31). Getting started with Metricbeat. Medium. Retrieved August 24, 2022, from https://medium.com/devops-dudes/getting-started-with-metricbeat-7c5718e6b3f4#:~:text=Metricbeat%20is%20a%20lightweight%20agent,beats%20and%20ELK%20stack%20itself.  
...
| Web-1                | DVWA Container | 20.9.54.192   | Linux            |
| Web-2                | DVWA Container | 20.9.54.192   | Linux            |
| Web-3                | DVWA Container | 20.9.54.192   | Linux            |
| ELK-SERVER           | ELK Container  | 20.127.31.165 | Linux            |
### Access Policies
...
- Which machine did you allow to access your ELK VM? What was its IP address?
  - VMs Web-1, Web-2, or Web-3 anyone machine that can be turned on can be used to access ELK-SERVER VM with following username and ip address:
    ...
    - command - ssh elkserveradmin@20.127.31.165 
    - Accessible - The elk server can be accessed by turning any one web vm on and sshing into the elk server because in the hosts file, ip address of all three web vms consists of the same elk docker container in them. So if web-2 is turned on and other two are turned off, kibana can be accessed that way too.

A summary of the access policies in place can be found in the table below.

| Name                 | Publicly Accessible   | Allowed IP Addresses       |
|----------------------|-----------------------|----------------------------|
| Jump-Box-Provisioner | Yes - SSH - 22        | 73.101.106.120             |
| Web-1                | No                    | RedTeamLBIP - 20.9.54.192  |
| Web-2                | No                    | RedTeamLBIP - 20.9.54.192  |
| Web-3                | No                    | RedTeamLBIP - 20.9.54.192  |
| ELK-SERVER           | Yes                   | HTTP API 9200 & 5601       |
### Elk Configuration
...
- Update the hosts file to include [elk] server attribute with the private ip of the elk server vm
